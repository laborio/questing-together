    ...
    <View style={[styles.card, embedded && styles.embeddedCard]}>
      ...
      >
        {actions.map((action) => {
          ...
          const hpLabel = hasHpDelta
            ? `HP ${action.hpDelta > 0 ? '+' : ''}${action.hpDelta}`
            : null;
            ...
            >
              ...
              {hpLabel || action.effectText ? (
                <View style={styles.actionMetaRow}>
                  {hpLabel ? (
                    <Text
                      style={[
                        ...
                        action.hpDelta && action.hpDelta < 0
                          ? styles.actionMetaNegative
                          : styles.actionMetaPositive,
                      ]}
                    >
                      {hpLabel}
                    </Text>
                  ...
                  {action.effectText ? (
                    <Text style={styles.actionMetaSubText}>{action.effectText}</Text>
                  ) : null}
                </View>
              ) : null}
              {action.isDisabled ? (
                <Text style={styles.actionDisabledText}>Unavailable after earlier actions</Text>
              ) : null}
            </Pressable>
          );
        })}
      ...
      {allowSkip ? (
        <Pressable
          disabled={!canAct}
          onPress={onSkip}
          style={[styles.skipButton, !canAct && styles.skipButtonDisabled]}
        >
