...
} from '@/features/party/ChoiceIntentPortraits';
import { SceneActionsCard } from '@/features/story/SceneActionsCard';
import { SceneOptionsCard } from '@/features/story/SceneOptionsCard';
...
export function DecisionPanelCard({
  ...
  if (embedded) {
    ...
    const timedIntroMessage =
      !timedEndsAt && isTimedScene ? <Text style={styles.voteLockedText}>{statusText}</Text> : null;
    return (
      <View style={styles.journalCard}>
        ...
        <View style={styles.buttonStack}>
          {actions.map((action) => {
            ...
            const hpDelta =
              typeof action.hpDelta === 'number' && Number.isFinite(action.hpDelta)
                ? action.hpDelta
                : 0;
            ...
            return (
              <Pressable
                ...
                style={[
                  ...
                  isSelectedAction && styles.textureButtonSelected,
                  isDisabled && !isSelectedAction && styles.textureButtonDisabled,
                ]}
              >
                <ImageBackground
                  source={
        ...
        {isCombatScene ? null : isTimedScene ? (
          ...
          <>
            ...
            {!canVote ? (
              <Text style={styles.voteLockedText}>{voteLockReason ?? 'Voting locked.'}</Text>
            ) : null}
            ...
            <View style={styles.buttonStack}>
              {visibleOptions.map((option) => {
                ...
                const isDisabled =
                  Boolean(resolvedOption) || Boolean(localConfirmedOption) || !canVote;
                const isLockedSelected =
                  Boolean(localConfirmedOption) && localConfirmedOption === option.id;
              ...
              {Array.from({ length: hiddenOptionCount }).map((_, index) => (
                <View
                  key={`hidden-${index}`}
                  style={[styles.textureButton, styles.textureButtonDisabled]}
                >
                  <ImageBackground
            ...
            {resolvedOption ? (
              <Text style={styles.waitingText}>
                Advancing when party confirmations are synced ({continuedCount}/
                {expectedPlayerCount}).
              </Text>
            ) : localConfirmedOption ? (
              <Text style={styles.waitingText}>
              ...
              <Pressable
                ...
                style={[
                  styles.textureButton,
                  (!draftOptionId || !canVote) && styles.textureButtonDisabled,
                ]}
              >
                <ImageBackground
                  source={
                    !draftOptionId || !canVote ? buttonTextureDisabled : buttonTextureSelected
                  }
                  style={styles.textureBg}
                  imageStyle={styles.textureImage}
                  resizeMode="stretch"
    ...
    <View style={[styles.card, embedded && styles.embeddedCard]}>
      ...
      {!isTimedScene && !isEndingScene ? (
        <View style={styles.tabsRow}>
          ...
          >
            <Text
              style={[styles.tabButtonText, activeTab === 'actions' && styles.tabButtonTextActive]}
            >
              Actions
            </Text>
          </Pressable>
          ...
          >
            <Text
              style={[
                ...
                activeTab === 'decisions' && styles.tabButtonTextActive,
              ]}
            >
