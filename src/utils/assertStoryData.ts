import type { OptionId, SceneMode, StoryData, TagCondition, TagSet } from '@/types/story';
...
function isTagCondition(value: unknown): value is TagCondition {
  ...
  return (
    (value.all === undefined || isStringArray(value.all)) &&
    (value.any === undefined || isStringArray(value.any)) &&
    (value.none === undefined || isStringArray(value.none))
  );
}
...
function isTagSet(value: unknown): value is TagSet {
  if (!isObject(value)) return false;
  return (
    (value.global === undefined || isStringArray(value.global)) &&
    (value.scene === undefined || isStringArray(value.scene))
  );
}

export function assertStoryData(raw: unknown): StoryData {
  if (!isObject(raw)) {
        ...
        combatObj.actions.forEach((action: unknown, actionIndex: number) => {
          ...
          if (!isObject(action.effect)) {
            ...
            if (effect.enemyAttackDelta !== undefined && !isFiniteNumber(effect.enemyAttackDelta)) {
              errors.push(
                `combat.actions[${actionIndex}].effect.enemyAttackDelta must be a number`,
              );
            }
  ...
  if (Array.isArray(scenes)) {
    scenes.forEach((scene, sceneIndex) => {
      ...
      if (typeof scene.title !== 'string')
        errors.push(`scenes[${sceneIndex}].title must be string`);
      ...
      if (typeof scene.intro !== 'string')
        errors.push(`scenes[${sceneIndex}].intro must be string`);
      ...
      const sceneMode: SceneMode =
        rawMode === 'combat' || scene.combat
          ? 'combat'
          : rawMode === 'timed' || scene.timed
            ? 'timed'
            : 'story';
      if (sceneMode === 'combat') {
        ...
        if (!Array.isArray(scene.introDialogue)) {
          ...
          scene.introDialogue.forEach((line: unknown, lineIndex: number) => {
            if (
              ...
              errors.push(
                `scenes[${sceneIndex}].introDialogue[${lineIndex}] must have speaker/text`,
              );
            }
          });
        }
        ...
        scene.steps.forEach((step: unknown, stepIndex: number) => {
          ...
          if (!Array.isArray(step.actions) || step.actions.length === 0) {
            ...
            step.actions.forEach((action: unknown, actionIndex: number) => {
              if (!isObject(action)) {
                errors.push(
                  `scenes[${sceneIndex}].steps[${stepIndex}].actions[${actionIndex}] must be object`,
                );
                return;
              }
              if (typeof action.id !== 'string') {
                errors.push(
                  `scenes[${sceneIndex}].steps[${stepIndex}].actions[${actionIndex}].id must be string`,
                );
                return;
              }
              ...
              if (typeof action.role !== 'string') {
                errors.push(
                  `scenes[${sceneIndex}].steps[${stepIndex}].actions[${actionIndex}].role must be string`,
                );
              }
              if (typeof action.text !== 'string') {
                errors.push(
                  `scenes[${sceneIndex}].steps[${stepIndex}].actions[${actionIndex}].text must be string`,
                );
              }
              if (action.buttonText !== undefined && typeof action.buttonText !== 'string') {
                errors.push(
                  `scenes[${sceneIndex}].steps[${stepIndex}].actions[${actionIndex}].buttonText must be string`,
                );
              }
              if (action.stage !== undefined && typeof action.stage !== 'string') {
                errors.push(
                  `scenes[${sceneIndex}].steps[${stepIndex}].actions[${actionIndex}].stage must be string`,
                );
              }
              if (action.narration !== undefined && typeof action.narration !== 'string') {
                errors.push(
                  `scenes[${sceneIndex}].steps[${stepIndex}].actions[${actionIndex}].narration must be string`,
                );
              }
            });
          }
          ...
          if (!isObject(step.outcomes)) {
            ...
            step.actions.forEach((action: Record<string, unknown>) => {
              const outcome = (step.outcomes as Record<string, unknown>)[action.id as string];
              if (!outcome || !isObject(outcome)) {
                errors.push(
                  `scenes[${sceneIndex}].steps[${stepIndex}].outcomes missing for action ${action.id}`,
                );
                return;
              }
                ...
                if (!Array.isArray(outcome.dialogue)) {
                  ...
                  outcome.dialogue.forEach((line: unknown, lineIndex: number) => {
                    if (
                      ...
                      errors.push(
                        `outcomes.${action.id}.dialogue[${lineIndex}] must have speaker/text`,
                      );
                      return;
                    }
                    if (line.aside !== undefined && typeof line.aside !== 'string') {
                      errors.push(
                        `outcomes.${action.id}.dialogue[${lineIndex}].aside must be string`,
                      );
                    }
                    if (line.narration !== undefined && typeof line.narration !== 'string') {
                      errors.push(
                        `outcomes.${action.id}.dialogue[${lineIndex}].narration must be string`,
                      );
                    }
                  });
                }
              ...
              if (isStringArray(outcome.evidenceIds)) {
                outcome.evidenceIds.forEach((id: string) => {
                  if (!evidenceIds.has(id)) {
                    errors.push(
                      `outcomes.${action.id}.evidenceIds references missing evidence ${id}`,
                    );
                  }
                });
              }
              if (
                outcome.unlockOptionIds !== undefined &&
                !isStringArray(outcome.unlockOptionIds)
              ) {
              ...
              if (
                outcome.disableActionIds !== undefined &&
                !isStringArray(outcome.disableActionIds)
              ) {
      ...
      if (Array.isArray(scene.options)) {
        ...
        scene.options.forEach((option: unknown, optionIndex: number) => {
          ...
          if (!Array.isArray(option.next) || option.next.length === 0) {
            errors.push(
              `scenes[${sceneIndex}].options[${optionIndex}].next must be non-empty array`,
            );
          } else {
            option.next.forEach((route: unknown, routeIndex: number) => {
              if (!isObject(route)) {
                errors.push(
                  `scenes[${sceneIndex}].options[${optionIndex}].next[${routeIndex}] must be object`,
                );
                return;
              }
              if (route.ifGlobal !== undefined && !isTagCondition(route.ifGlobal)) {
                errors.push(
                  `scenes[${sceneIndex}].options[${optionIndex}].next[${routeIndex}].ifGlobal invalid`,
                );
              }
              if (route.ifScene !== undefined && !isTagCondition(route.ifScene)) {
                errors.push(
                  `scenes[${sceneIndex}].options[${optionIndex}].next[${routeIndex}].ifScene invalid`,
                );
              }
              if (route.ifActions !== undefined && !isTagCondition(route.ifActions)) {
                errors.push(
                  `scenes[${sceneIndex}].options[${optionIndex}].next[${routeIndex}].ifActions invalid`,
                );
              }
              if (route.ifActions && isTagCondition(route.ifActions)) {
                ...
                actionRefs.forEach((id: string) => {
                  if (!actionIds.has(id)) {
                    errors.push(
                      `scenes[${sceneIndex}].options[${optionIndex}].next[${routeIndex}].ifActions references missing action ${id}`,
                    );
                  }
                });
              }
              pendingRoutes.push({
                ...
                optionId: option.id as OptionId,
                to: route.to,
              });
            });
          }
      ...
      if (!Array.isArray(scene.unlockRules)) {
        ...
        scene.unlockRules.forEach((rule: unknown, ruleIndex: number) => {
          ...
          if (!optionIds.has(rule.optionId)) {
            errors.push(
              `scenes[${sceneIndex}].unlockRules[${ruleIndex}] references missing option`,
            );
          }
          rule.evidenceIds.forEach((id: string) => {
            if (!evidenceIds.has(id)) {
              errors.push(
                `scenes[${sceneIndex}].unlockRules[${ruleIndex}] references missing evidence ${id}`,
              );
            }
          });
        });
      ...
      if (!isObject(scene.outcomeByOption)) {
        ...
        optionIds.forEach((optionId) => {
          ...
          if (!outcome || !isObject(outcome) || typeof outcome.text !== 'string') {
            ...
            if (outcome.hpDelta !== undefined && !isFiniteNumber(outcome.hpDelta)) {
              errors.push(
                `scenes[${sceneIndex}].outcomeByOption.${optionId}.hpDelta must be number`,
              );
            }
          }
        });
  ...
  pendingRoutes.forEach((route) => {
    ...
    if (typeof route.to !== 'string' || !sceneIds.has(route.to)) {
      errors.push(
        `Scene ${route.sceneId} option ${route.optionId} routes to missing scene ${String(route.to)}`,
      );
    }
  });
  ...
  if (combatScenes.length > 0 && !combat) {
    errors.push(
      `combat config is required when combat scenes exist (missing for ${combatScenes.join(', ')})`,
    );
  }
