...
import { MAX_PARTY_EMOTES_PER_SCENE, PARTY_EMOTES } from '@/src/game/constants';
import { EmoteText, PartyEmote, PlayerId } from '@/src/game/types';
...
export function usePartyEmotes({ localPlayerId, roomId, currentSceneId }: UsePartyEmotesOptions) {
  ...
  const appendRows = useCallback((rows: RoomMessageRow[], shouldToast: boolean) => {
    ...
    const mapped = rows.map(toPartyEmote).filter((message): message is PartyEmote => Boolean(message));
  ...
  useEffect(() => {
    ...
    const loadInitialRoomMessages = async () => {
      ...
      const initialMessages = rows.map(toPartyEmote).filter((message): message is PartyEmote => Boolean(message));
